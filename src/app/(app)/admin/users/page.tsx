...
export default async function UsersPage() {
  ...
  return (
    <div className="mx-auto max-w-4xl px-4 py-6 space-y-4">
      ...
      <form
        action={createUser}
        className="bg-white rounded-2xl border border-[#E2E0DA] p-4 grid grid-cols-1 md:grid-cols-5 gap-3 items-end"
      >
        <label className="flex flex-col gap-1">
          <span className="text-xs text-[#7A7A7A]">账号</span>
          <input name="username" required className="select" />
        </label>
        <label className="flex flex-col gap-1">
          <span className="text-xs text-[#7A7A7A]">密码 (≥6 位)</span>
          <input
            name="password"
            type="text"
            required
        ...
        <label className="flex flex-col gap-1">
          <span className="text-xs text-[#7A7A7A]">显示名</span>
          <input name="displayName" className="select" />
        </label>
        <label className="flex flex-col gap-1">
          <span className="text-xs text-[#7A7A7A]">权限</span>
      ...
      <div className="bg-white rounded-2xl border border-[#E2E0DA] overflow-x-auto">
        <table className="w-full text-sm">
          ...
          <tbody>
            {users.map((u) => (
              <tr key={u.id} className="border-b border-[#F0EFE9]">
                ...
                <td className="p-3">
                  {meId === u.id ? (
                    <span className="text-xs text-[#BDBDBD]">(当前账号)</span>
                  ) : (
                    <form
                      action={async () => {
                        "use server";
